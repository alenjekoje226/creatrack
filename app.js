...
const STORAGE_KEY_BASE = 'creatrack_data_v1';
let currentUser = null;

function storageKeyForUser() {
  // Each Google account gets its own data slice on this device
  return currentUser ? `${STORAGE_KEY_BASE}_${currentUser.uid}` : `${STORAGE_KEY_BASE}_guest`;
}
...
function saveData() {
  try {
    localStorage.setItem(storageKeyForUser(), JSON.stringify(data));
  } catch (e) {
...
function renderUserPill(user) {
  ...
  if (!header || document.getElementById('user-pill')) return;
  const pill = document.createElement('button');
  pill.id = 'user-pill';
  pill.className = 'user-pill';
  const label = user.isAnonymous ? 'Guest' : (user.email ? user.email.split('@')[0] : 'Account');
  const initial = user.isAnonymous ? 'G' : (user.email || '?').charAt(0).toUpperCase();
  pill.innerHTML = `
    <span class="user-pill-avatar">${initial}</span>
    <span>${label}</span>
  `;
  pill.addEventListener('click', () => {
    if (confirm('Sign out of CreaTrack?')) {
      window.__signOutFirebase();
    }
  });
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (signInBtn) {
    signInBtn.addEventListener('click', () => {
      ...
      window.__signInWithEmail(email, password);
    });
  }
  ...
  if (signUpBtn) {
    signUpBtn.addEventListener('click', () => {
      ...
      errEl.classList.add('hidden');
      window.__signUpWithEmail(email, password);
    });
  }
  ...
  window.addEventListener('creatrack-auth-state-changed', (e) => {
    const user = e.detail;
    if (user) {
      handleSignedIn(user);
    } else {
  ...
  window.addEventListener('creatrack-auth-error', (e) => {
    showLoginError('Sign-in failed: ' + e.detail);
  });
})
